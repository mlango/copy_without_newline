...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.input_edit.setFixedHeight(100)  # 设置输入框高度
        self.output_label = QLabel("处理后的文本：")
        self.output_edit = QLineEdit()
        self.output_edit.setFixedHeight(100)  # 设置输出框高度
    ...
    def process_text(self):
        text = self.input_edit.text()  # 获取输入框中的文本
        text = text.replace('\n', ' ')  # 将换行符替换为空格
        self.output_edit.setText(text)  # 将处理后的文本显示在输出框中

    def copy_output(self):
        text = self.output_edit.text()  # 获取输出框中的文本
